...
* 素の JavaScript
* Tailwind CSS
* TypeScript (ユニットテストでのみ)

=== ユニットテスト

テストを実行すると、コードフォーマットと Lint も同時に実行される。
...
以下のコマンドで http://localhost:8000 で画面を確認できる。
`deno task` を見れば分かるが、public ディレクトリで
python の http.server を起動してるだけなんで、サーバ起動できれば何を使っても良い。
